...
int usbd_msc_sector_read(uint8_t busid, uint8_t lun, uint32_t sector, uint8_t *buffer, uint32_t length)
{
    ...
    if (sector == 0) {
        memcpy(buffer, fake_mbr, length);
               ...
               ((sector >= 0xFFFFFFD8ul) && (sector < 0xFFFFFFD8ul + 24))) {
        /* exFAT 引导区 24 扇区（主 12 + 备份 12）：扇区 0/12 = BPB(512B 数组)，
         * 1-8/13-20 = 扩展签名 AA550000h，11/23 = 校验和扇区（0x860AFC84×128），其余 0 */
        unsigned long off = (sector >= FAKE_PART_LBA) ? (sector - FAKE_PART_LBA) : (sector - 0xFFFFFFD8ul);
        uint32_t ss = (uint32_t)(off % 12);
        if (ss == 0) {
            ...
            for (i = 0; i < length; i += 4) {
                buffer[i] = 0x84; buffer[i+1] = 0xFC; buffer[i+2] = 0x0A; buffer[i+3] = 0x86;
            }
        } else {
            memset(buffer, 0, length);
        }
        ...
        /* 簇位图（簇 2，DataLength=524284B）：簇 2/3/4 已分配（首字节 0x1C） */
        memset(buffer, 0, length);
        if (sector == 34880) {
            buffer[0] = 0x1C;
        }
    } else if ((sector >= 36928) && (sector < 36928 + 12)) {
        /* Up-case 表（簇 4，压缩格式 60B） */
        if (sector == 36928) {
            memset(buffer, 0, length);
